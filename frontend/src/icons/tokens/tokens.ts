import Aberration from "./Aberration.jpg";
import Artificer from "./Artificer.jpg";
import Barbarian from "./Barbarian.jpg";
import Bard from "./Bard.jpg";
import Beast from "./Beast.jpg";
import BloodHunter from "./BloodHunter.jpg";
import Celestial from "./Celestial.jpg";
import Cleric from "./Cleric.jpg";
import Construct from "./Construct.jpg"
import Dragon from "./Dragon.jpg";
import Druid from "./Druid.jpg";
import Elemental from "./Elemental.jpg";
import Fey from "./Fey.jpg";
import Fiend from "./Fiend.jpg";
import Fighter from "./Fighter.jpg";
import Giant from "./Giant.jpg";
import Goblinoid from "./Goblinoid.jpg";
import Humanoid from "./Humanoid.jpg";
import Monk from "./Monk.jpg";
import Monstrosity from "./Monstrosity.jpg"
import Ooze from "./Ooze.jpg";
import Paladin from "./Paladin.jpg";
import Plant from "./Plant.jpg";
import Ranger from "./Ranger.jpg";
import Rogue from "./Rogue.jpg";
import Shapechanger from "./Shapechanger.jpg";
import Sorcerer from "./Sorcerer.jpg";
import Titan from "./Titan.jpg"
import Undead from "./Undead.jpg";
import Warlock from "./Warlock.jpg";
import Wizard from "./Wizard.jpg";

export const images = [
  /*{/* Aberration /*}*/,
  /*{*/ Artificer /*}*/,
  /*{*/ Barbarian /*}*/,
  /*{*/ Bard /*}*/, Beast, BloodHunter, Celestial, Cleric, Construct, Dragon, Druid, Elemental, Fey, Fiend, Fighter, Giant, Goblinoid, Humanoid, Monk, Monstrosity, Ooze, Paladin, Plant, Ranger, Rogue, Shapechanger, Sorcerer, Titan, Undead, Warlock, Wizard]